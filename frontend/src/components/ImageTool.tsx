...
import { Plus, Upload, RotateCw, Image as ImageIcon, Sparkles } from 'lucide-react';
...
export function ImageTool({ onAddElement, selectedElement, onUpdateElement }: ImageToolProps) {
  ...
  const [opacity, setOpacity] = useState(selectedElement?.opacity || 100);
  const [removeBg, setRemoveBg] = useState(selectedElement?.removeBg || false);
  const [removeBgDeep, setRemoveBgDeep] = useState(selectedElement?.removeBgDeep || 0);
  const [removeBgMode, setRemoveBgMode] = useState<'light' | 'dark'>(selectedElement?.removeBgMode || 'light');
  ...
  const handleAddFromGallery = (url: string, name: string) => {
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2 p-2 bg-gray-50 rounded-xl border border-gray-100 max-h-48 overflow-y-auto">
            {userImages.map((img) => (
              <button
                key={img.id}
                onClick={() => handleAddFromGallery(img.value, img.name)}
        ...
        <div className="p-4 bg-indigo-50 rounded-xl border border-indigo-100 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex flex-col">
              <Label className="text-sm font-bold text-indigo-900">Background Removal</Label>
              <p className="text-[10px] text-indigo-600">Apply magic filters or AI</p>
            </div>
          </div>
          ...
          <Tabs
            ...
            className="w-full"
          >
            <TabsList className="grid w-full grid-cols-2 h-8 p-1 bg-white/50 border border-indigo-100 rounded-lg">
            ...
            <TabsContent value="js" className="space-y-4 mt-3">
              <div className="flex items-center justify-between">
                <Label className="text-xs font-semibold text-indigo-700">Enable Filter</Label>
                <Switch
                ...
                <div className="space-y-4 pt-2">
                  <div>
                    <div className="flex justify-between mb-1.5">
                      <Label className="text-[10px] font-bold text-indigo-700">Sensitivity</Label>
                  ...
                  <div>
                    <Label className="text-[10px] font-bold text-indigo-700 mb-1.5 block">Target Color</Label>
                    <Select
                      value={selectedElement.removeBgMode || 'light'}
                      onValueChange={(val: any) => handleUpdate({ removeBgMode: val })}
                    >
                      <SelectTrigger className="h-8 text-[10px] bg-white border-indigo-100">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
