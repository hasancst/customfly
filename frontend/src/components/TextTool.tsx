...
import { Plus, Type, ChevronDown, Layers, Settings2 } from 'lucide-react';
...
export function TextTool({ onAddElement, selectedElement, onUpdateElement }: TextToolProps) {
  ...
  const [fontFamily] = useState(selectedElement?.fontFamily || 'Inter');
  const [fontWeight] = useState(selectedElement?.fontWeight || 400);
  const [textAlign] = useState<'left' | 'center' | 'right'>(selectedElement?.textAlign || 'center');
  const [color] = useState(selectedElement?.color || '#000000');
  ...
  useEffect(() => {
    if (selectedElement) {
      ...
      if (selectedElement.textMode) setTextMode(selectedElement.textMode);
      if (selectedElement.maxChars) setMaxChars(selectedElement.maxChars);
      if (selectedElement.textCase) setTextCase(selectedElement.textCase);
      if (selectedElement.textType) setTextType(selectedElement.textType);
    } else {
      setText('');
      setMaxChars(0);
      setTextCase('none');
  ...
  const handleAddText = (bridge?: any, isMonogram?: boolean, shape?: any) => {
    ...
    // Default text for new elements if input is empty
    if (!finalText.trim()) {
      finalText = 'Your Text Here';
    }
    ...
    const newElement: CanvasElement = {
      ...
      width: 300,
      height: 100,
      fontSize: isMonogram ? 100 : fontSize,
      fontFamily: selectedFont,
      fontWeight: isMonogram ? 400 : fontWeight,
      textAlign,
      color,
      rotation: 0,
      opacity: 100,
  ...
  const formatText = (val: string, currentTextType: string, currentTextCase: string, currentMaxChars: number) => {
    ...
    // 1. Filter by type
    if (currentTextType === 'numbers') {
      formatted = formatted.replace(/[^0-9]/g, '');
    }

    // 2. Case transformation
    if (currentTextCase === 'uppercase') {
      formatted = formatted.toUpperCase();
    } else if (currentTextCase === 'lowercase') {
      formatted = formatted.toLowerCase();
    }

    // 3. Limit length
    if (currentMaxChars > 0) {
      formatted = formatted.substring(0, currentMaxChars);
    }
  ...
  return (
    <div className="space-y-6 pb-4">
      {/* Main Text Input Field - Common for all types */}
      <div className="px-1 space-y-2">
        ...
        <Collapsible className="mt-4">
          ...
          <CollapsibleContent className="pt-3 space-y-4 px-1">
            ...
            {/* Content Type */}
            <div className="space-y-1.5">
              <Label className="text-[10px] font-bold text-gray-400 uppercase">Content Restriction</Label>
              <ToggleGroup
                type="single"
                value={textType}
                onValueChange={(val: any) => {
                  if (val) {
                    setTextType(val);
                    if (selectedElement) {
                      const newText = formatText(text, val, textCase, maxChars);
                      setText(newText);
                      handleUpdate({ textType: val, text: newText });
                    }
                  }
                }}
                className="w-full bg-gray-50 p-0.5 rounded-lg border border-gray-100"
              >
                <ToggleGroupItem value="all" className="flex-1 h-7 rounded-md text-[10px] font-bold data-[state=active]:bg-white">
                  <TypeIcon className="w-3.5 h-3.5 mr-1" /> Regular
                </ToggleGroupItem>
                <ToggleGroupItem value="numbers" className="flex-1 h-7 rounded-md text-[10px] font-bold data-[state=active]:bg-white">
                  <Hash className="w-3.5 h-3.5 mr-1" /> Numbers only
                </ToggleGroupItem>
              </ToggleGroup>
            </div>
          </CollapsibleContent>
