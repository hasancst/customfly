...
import { ImageIcon, CloudUpload, ShoppingBag, CheckCircle2, LayoutGrid, Layers } from 'lucide-react';
...
export function BaseImageModal({
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (event) => {
            ...
            onSelectImage(url, false, activeTargetId);
        };
                ...
                <DialogHeader className="p-6 pb-4 border-b border-gray-100 shrink-0">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-sm text-gray-500 mt-1">Assign mockups to specific variants to automate scene changes.</p>
                        </div>
                                ...
                                {productData?.variants.map((variant) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`w-full text-left px-3 py-2 rounded-xl transition-all flex items-center gap-3 ${isSelected ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'hover:bg-gray-100 text-gray-600'}`}
                                        >
                                            <div className="w-8 h-8 rounded-lg overflow-hidden shrink-0 border border-gray-200 bg-white">
                                                {assignedImage ? (
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className={`text-[9px] truncate ${isSelected ? 'text-indigo-100' : 'text-gray-400'}`}>
                                                    {variantBaseImages[variant.id] ? 'Custom Assignment' : (variant.image ? 'Standard Image' : 'No Mockup')}
                                                </div>
                                            </div>
                                            {variantBaseImages[variant.id] && (
                                            ...
                                            {(() => {
                                                ...
                                                return v?.image ? (
                                                    <div
                                                        ...
                                                        className={`group relative aspect-square w-32 rounded-2xl border-2 transition-all cursor-pointer overflow-hidden ${variantBaseImages[activeTargetId] === v.image ? 'border-indigo-500 ring-4 ring-indigo-500/10' : 'border-gray-100 hover:border-indigo-300 shadow-sm'}`}
                                                    >
                                                        ...
                                                        {variantBaseImages[activeTargetId] === v.image && (
                                    ...
                                    <div>
                                        ...
                                        <div className="grid grid-cols-4 gap-4">
                                            {productData?.images.map((img, idx) => {
                                                ...
                                                const isVariantActive = activeTargetId !== 'all' && variantBaseImages[activeTargetId] === img;
