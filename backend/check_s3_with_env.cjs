  ...
  for (const folder of folders) {
    ...
    try {
      ...
      if (oldPathResponse.Contents && oldPathResponse.Contents.length > 0) {
        console.log(`  ⚠ Old path (${folder}/${SHOP}/): ${oldPathResponse.Contents.length} files (needs migration)`);
        oldPathResponse.Contents.slice(0, 3).forEach(obj => {
          console.log(`    - ${obj.Key}`);
        });
      }
      ...
      if (rootPathResponse.Contents && rootPathResponse.Contents.length > 0) {
        ...
        if (rootFiles.length > 0) {
          console.log(`  ⚠ Root path (${folder}/): ${rootFiles.length} files (needs migration)`);
          rootFiles.slice(0, 3).forEach(obj => {
            console.log(`    - ${obj.Key}`);
          });
        }
      }
